...
TEST(Bounds, PointDistance) {
  {
    ...
    // General points
    EXPECT_EQ(3 * 3 + 7 * 7 + 10 * 10, distance_squared(Point3f(4, 8, -10), b));
    EXPECT_EQ(6 * 6 + 10 * 10 + 7 * 7, distance_squared(Point3f(-6, -10, 8), b));
  }

  {
    ...
    EXPECT_EQ(2 * 2 + 6 * 6 + 4 * 4, distance_squared(Point3f(-3, -9, 22), b));
  }
}
